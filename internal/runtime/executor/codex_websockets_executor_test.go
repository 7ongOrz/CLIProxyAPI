...
func (c *websocketReadDeadlineConn) Write(payload []byte) (int, error) {
	c.mu.Lock()
	delay := c.writeDelay
	c.mu.Unlock()
	if delay > 0 {
...
func (c *websocketReadDeadlineConn) setWriteDelay(delay time.Duration) {
	c.mu.Lock()
	c.writeDelay = delay
	c.mu.Unlock()
}
...
func TestCodexWebsocketRequestRenewsReadDeadlineAfterWrite(t *testing.T) {
	...
	trackedConn.setWriteDelay(100 * time.Millisecond)
	if errWrite := writeCodexWebsocketMessage(&codexWebsocketSession{}, conn, []byte(`{"type":"response.create"}`)); errWrite != nil {
	...
	wantDeadline := afterWrite.Add(codexResponsesWebsocketIdleTimeout - 20*time.Millisecond)
	if renewedDeadline.Before(wantDeadline) {
		t.Fatalf("read deadline = %v, want at least %v", renewedDeadline, wantDeadline)
	}
...
func TestParseCodexWebsocketResponseFailedInfersStatus(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			streamErr, ok := parseCodexResponseFailed([]byte(tt.payload))
			if !ok {
