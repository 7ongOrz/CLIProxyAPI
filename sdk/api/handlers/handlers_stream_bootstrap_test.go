...
func TestAuthManagerStreamResultErrorMarksFailureAndPreservesRetryAfter(t *testing.T) {
	...
	if string(chunk.Payload) != "partial" || chunk.Err != nil || chunk.ResultErr != nil || chunk.ResultStatusCode != 0 {
		t.Fatalf("forwarded chunk = payload %q err=%v result_err=%v result_status=%d", chunk.Payload, chunk.Err, chunk.ResultErr, chunk.ResultStatusCode)
	}
	chunk, ok = <-result.Chunks
	if !ok {
		t.Fatal("stream closed before result error payload")
	}
	if string(chunk.Payload) != "failed" || chunk.Err != nil || chunk.ResultErr != nil || chunk.ResultStatusCode != http.StatusTooManyRequests {
		t.Fatalf("result error chunk = payload %q err=%v result_err=%v result_status=%d", chunk.Payload, chunk.Err, chunk.ResultErr, chunk.ResultStatusCode)
	}
...
func TestAuthManagerBootstrapResultErrorSeparatesClientAndAccounting(t *testing.T) {
	...
	if string(chunk.Payload) != clientPayload || chunk.Err != nil || chunk.ResultErr != nil || chunk.ResultStatusCode != http.StatusTooManyRequests {
		t.Fatalf("client chunk = payload %q err=%v result_err=%v result_status=%d", chunk.Payload, chunk.Err, chunk.ResultErr, chunk.ResultStatusCode)
	}
	if _, ok = <-result.Chunks; ok {
...
func TestExecuteStreamWithAuthManager_RetriesBootstrapResultErrorWithCoolingDisabled(t *testing.T) {
	...
	if string(chunk.Payload) != successPayload || chunk.Err != nil || chunk.ResultErr != nil || chunk.ResultStatusCode != 0 {
		t.Fatalf("client chunk = payload %q err=%v result_err=%v result_status=%d", chunk.Payload, chunk.Err, chunk.ResultErr, chunk.ResultStatusCode)
	}
	if _, ok = <-result.Chunks; ok {
